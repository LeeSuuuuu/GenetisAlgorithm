...
# 시간대 별 총 범죄 발생 수
공공데이터의 대검찰청 범죄 발생시간 2017.12.31 데이터 사용
...
* 그래프의 y축 (종속 변수)  
범죄 발생 수: 자료에 나와있는 범죄의 총 합

## 그래프를 좌표로 표현하기
(x, y):  
(1, 102151) (2, 97195) (3, 126474) (4, 123823)  
(5, 151793) (6, 175333) (7, 288786) (8, 31587)

## 기타 설정 설명
* 1차모수식을 구할 때 y절편을 75000으로 고정하여 기울기(모수 a)만 구함
* 염색체 범위는 32767로 2진로 2의 12승 자리 나타냄
...
* 에러값을 줄이기 위해 염색체의 범위를 32->32767로 변경
* 그래프에 대한 값들을 불러오기 위해 txt파일을 불러옴
* 불러온 파일을 구조체 배열에 저장

## 사진
### 0~2 돌연변이 그림
(출력은 제대로 되나 염색체 뽑을때 범위가 너무 작았음)  
1번 인덱스를 기준을 교차연산하고 돌연변이를 0~2로 다르게 출력 했음.  
...
### 5cut 6mut그림
5번째 인덱스를 기준으로 교차연산을 하고 6번째 인덱스를 돌연변이 연산을 해줌 (3번 회전)  
결과: 2번째 회전의 유사값(13970)이 모수값으로 가장 적합함.  

### 결과 그래프
5cut 6mut에서 나온 모수값(13970)을 y=ax+75000의 a에 대입한 그래프 (주황색 선)와 시간-범죄 그래프(파란색 점)를 같이 나타난 그래프  


# 아쉬운 점
노트북에 과부하가 와서 많은 회전을 시키지 못해 효율을 구하지 못함
